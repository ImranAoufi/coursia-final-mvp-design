...
const Enterprise = () => {
  ...
  const handleBookDemo = () => {
    // Öffnet das externe Demo-Formular in einem neuen Tab
    window.open("https://calendly.com/deine-demo", "_blank");
  };
  ...
  return (
    <div className="min-h-screen">
      ...
      {/* Sticky Header */}
      <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? "glass py-3" : "py-6"}`}>
        <div className="container mx-auto px-6 flex items-center justify-between">
          <Logo className="h-16 md:h-20 object-contain cursor-pointer" onClick={() => navigate("/")} />

          <nav className="hidden md:flex items-center gap-8">
            <button onClick={() => navigate("/#product")} className="text-foreground/80 hover:text-foreground transition-colors">Product</button>
            <button onClick={() => navigate("/#learn")} className="text-foreground/80 hover:text-foreground transition-colors">Learn</button>
            <button onClick={() => navigate("/pricing")} className="text-foreground/80 hover:text-foreground transition-colors">Pricing</button>
            <button onClick={() => navigate("/enterprise")} className="text-foreground hover:text-foreground transition-colors font-semibold">Enterprise</button>
          </nav>
      ...
      <section className="relative pt-32 pb-20 px-6 overflow-hidden">
        <div className="container mx-auto max-w-6xl text-center relative z-10">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4 animate-slide-up relative z-20" style={{ animationDelay: "0.2s" }}>
            <Button variant="gradient" size="lg" onClick={handleBookDemo} className="text-lg px-12 py-6 h-auto group shadow-glow">
              Book a Demo
              <ArrowRight className="ml-2 group-hover:translate-x-1 transition-transform" />
            </Button>
            <Button variant="glass" size="lg" onClick={handleBookDemo} className="text-lg px-12 py-6 h-auto">
              Talk to Sales
            </Button>
          </div>
      ...
      <section className="py-24 px-6 relative">
        <div className="container mx-auto max-w-7xl">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[
              { icon: Building2, title: "Custom Branding", description: "Own domain, white-label design, and fully branded dashboards that reflect your organization's identity", color: "primary" },
              { icon: Users, title: "Multi-user Access", description: "Manage multiple instructors, teams, or departments with granular permissions and role-based access control", color: "secondary" },
              { icon: ShoppingBag, title: "Private Course Marketplaces", description: "Create internal or external course marketplaces with flexible monetization and payment processing", color: "accent" },
              { icon: BarChart3, title: "Advanced Analytics", description: "Deep learner insights, engagement tracking, and AI-driven recommendations to optimize learning outcomes", color: "primary" },
              { icon: Bot, title: "Dedicated AI Support", description: "Fine-tuned AI model trained on your organization's tone, style, and content standards", color: "secondary" },
              { icon: Sparkles, title: "Priority Support", description: "Dedicated success manager, 24/7 technical support, and custom integration assistance", color: "accent" }
            ].map((feature, idx) => (
              <div key={idx} className="glass-strong rounded-2xl p-8 hover:scale-[1.02] transition-all duration-300 group cursor-pointer animate-slide-up" style={{ animationDelay: `${idx * 0.1}s` }}>
        ...
        <div className="container mx-auto max-w-6xl">
          <div className="glass-strong rounded-3xl p-12 relative overflow-hidden">
            ...
            <div className="relative z-10 grid md:grid-cols-4 gap-8 text-center">
              {[{ value: "50+", label: "Enterprise Clients" }, { value: "500K+", label: "Active Learners" }, { value: "10M+", label: "Courses Generated" }, { value: "99.9%", label: "Uptime SLA" }].map((stat, idx) => (
      ...
      <footer className="glass border-t border-glass-border py-8 px-6">
        <div className="container mx-auto flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <nav className="flex items-center gap-6">
            <button onClick={() => navigate("/#product")} className="text-muted-foreground hover:text-foreground text-sm transition-colors">Product</button>
            <button onClick={() => navigate("/#learn")} className="text-muted-foreground hover:text-foreground text-sm transition-colors">Learn</button>
            <button onClick={() => navigate("/pricing")} className="text-muted-foreground hover:text-foreground text-sm transition-colors">Pricing</button>
            <button onClick={() => navigate("/enterprise")} className="text-muted-foreground hover:text-foreground text-sm transition-colors">Enterprise</button>
          </nav>
        </div>
      </footer>
    </div>
