...
import { normalizeCourse, NormalizedCourse } from "@/lib/normalizeCourse";
...
const MyCourse = () => {
    ...
    const [course, setCourse] = useState<FullCourse | null>(() => {
        ...
        return normalizeCourse(raw) as FullCourse;
    });
        ...
        const poll = async () => {
            try {
                ...
                if (!cancelled && result) {
                    ...
                    const normalized = normalizeCourse(result) as FullCourse;
                    setCourse(normalized);
                    setStatus("done");
                    sessionStorage.setItem("coursia_full_course", JSON.stringify(normalized));
    ...
    const handleViewScript = async (title: string, path?: string, lessonIndex?: number, inlineContent?: string) => {
        // Use inline content if available (from fallback/AI generation)
        if (inlineContent) {
            sessionStorage.setItem("coursia_edit_script", inlineContent);
            navigate(`/my-course/script?title=${encodeURIComponent(title)}&lessonIndex=${lessonIndex ?? 0}&courseId=${jobId || ""}`);
            return;
        }
        if (!path || path === "__inline__") return alert("No script file found.");
    ...
    const handleViewQuiz = async (title: string, path?: string, lessonIndex?: number, inlineData?: any) => {
        // Use inline quiz data if available
        if (inlineData) {
            const quizUpdates = JSON.parse(sessionStorage.getItem("coursia_quiz_updates") || "{}");
            const quizData = (lessonIndex !== undefined && quizUpdates[lessonIndex]) ? quizUpdates[lessonIndex] : inlineData;
        ...
        if (!path || path === "__inline__") return alert("No quiz file found.");
        try {
            const res = await fetch(toURL(path));
            ...
            if (lessonIndex !== undefined) {
                const quizUpdates = JSON.parse(sessionStorage.getItem("coursia_quiz_updates") || "{}");
                if (quizUpdates[lessonIndex]) quizData = quizUpdates[lessonIndex];
            }
            sessionStorage.setItem("coursia_edit_quiz", JSON.stringify(quizData));
            navigate(`/my-course/quiz?title=${encodeURIComponent(title)}&lessonIndex=${lessonIndex ?? 0}`);
        } catch (err) {
    ...
    const handleViewWorkbook = async (title: string, path?: string, lessonIndex?: number, inlineData?: any) => {
        // Use inline workbook data if available
        if (inlineData) {
            const text = inlineData.prompts ? inlineData.prompts.join("\n\n") : JSON.stringify(inlineData);
        ...
        if (!path || path === "__inline__") return alert("No workbook file found.");
        try {
            const res = await fetch(toURL(path));
                ...
                <PreLaunchChecklist
                    ...
                    onConfirmPublish={async (data: PreLaunchData) => {
                        ...
                        for (const lesson of course.lessons || []) {
                            // Use inline data first, then try file fetch
                            if ((lesson as any).quiz_data) {
                                quizzes.push((lesson as any).quiz_data);
                            } else if (lesson.quiz_file && lesson.quiz_file !== "__inline__") {
                                try {
                            ...
                            if ((lesson as any).workbook_data) {
                                workbooks.push((lesson as any).workbook_data.prompts?.join("\n\n") || "");
                            } else if (lesson.workbook_file && lesson.workbook_file !== "__inline__") {
                                try {
                                                                        ...
                                                                        <div className="flex-1 min-w-0">
                                                                            ...
                                                                            <Button
                                                                                ...
                                                                                onClick={() => handleViewScript(
                                                                                    video.title || `Video ${vi + 1}`,
                                                                                    video.script_file,
                                                                                    li,
                                                                                    video.script_content || lesson.script_content
                                                                                )}
                                                                            >
                                                    ...
                                                    {lesson.quiz_file && (
                                                        <div className="p-4 rounded-xl bg-gradient-to-br from-emerald-500/10 to-teal-500/10 border border-emerald-500/20">
                                                            ...
                                                            <Button
                                                                ...
                                                                onClick={() => handleViewQuiz(lesson.lesson_title || "Quiz", lesson.quiz_file, li, (lesson as any).quiz_data)}
                                                            >
                                                    ...
                                                    {lesson.workbook_file && (
                                                        <div className="p-4 rounded-xl bg-gradient-to-br from-secondary/10 to-purple-500/10 border border-secondary/20">
                                                            ...
                                                            <Button
                                                                ...
                                                                onClick={() => handleViewWorkbook(lesson.lesson_title || "Workbook", lesson.workbook_file, li, (lesson as any).workbook_data)}
                                                            >
                                                                Open Workbook
                                                            </Button>
                                                        </div>
